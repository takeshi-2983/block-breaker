...
const imageAssets = ['manta', 'cat', 'gorilla', 'monkey'];

imageAssets.forEach(char => {
    const img = new Image();
    ...
    img.onerror = () => {
        console.log(`Failed to load ${char} image, will use fallback drawing`);
    };
});
...
function drawCharacter() {
    ...
    if (characterImages[selectedCharacter]) {
        const img = characterImages[selectedCharacter];
        const imgWidth = paddle.width * 1.5 * scaleFactor;
        const imgHeight = 50 * scaleFactor;
        const imgX = paddle.x + paddle.width / 2 - imgWidth / 2;
        const imgY = paddle.y - imgHeight / 2;
        ...
        ctx.globalAlpha = 0.8;
        ctx.fillRect(paddle.x, paddle.y - 10, paddle.width * scaleFactor, 20 * scaleFactor);
        ctx.globalAlpha = 1;
